...
Requires: cpp gcc glib-ghash imake subversion rpm-build autoconf automake re2c glib-devel file-libs popt-devel rpm-devel libstdc++-devel zlib-devel gcc-c++ libxml2-devel pam-devel glib2-devel flex bison cmake flex-devel php-devel libdb-devel python3-devel rpm-sign perl-generators golang gettext valkey-devel

# Modified for EL9: python2-devel changed to python3-devel

# Removed for EL8: file-devel (replaced with file-libs, but beware missing magic.h)
...
* Fri Mar 27 2026 Michael Stauber <mstauber@solarspeed.net> 0.7.2-3
- Updated module.mk

* Fri Jun 20 2025 Michael Stauber <mstauber@solarspeed.net> 0.7.2-2
- Updated module.mk

* Sat Jun 07 2025 Michael Stauber <mstauber@solarspeed.net> 0.7.2-1
- Replaced -Wl,-rpath with -Wl,-rpath-link in rules.mk to avoid RPM 
  build failures due to invalid RPATH
- No functional change to built binaries; only affects shared library 
  linking for packaging compliance

* Wed May 13 2025 Michael Stauber <mstauber@solarspeed.net> 0.7.1-2
- Added 'gettext' as requirement. Somehow it went walkies

* Wed May 13 2025 Michael Stauber <mstauber@solarspeed.net> 0.7.1-1
- Added 'golang' as requirement

* Mon Feb 26 2024 Michael Stauber <mstauber@solarspeed.net> 0.7.0-0BX03
- Updated requirements for perl-generators

* Mon Feb 19 2024 Michael Stauber <mstauber@solarspeed.net> 0.7.0-0BX02
- Version number bump for Elmer release

* Wed Nov 16 2022 Michael Stauber <mstauber@solarspeed.net> 0.7.0-0BX01
- Production ready build for BlueOnyx 5211R with new directory layout
- Added /usr/sausalito/bin/ucfirst.pl as new dependency for module.mk

* Wed Nov 10 2021 Michael Stauber <mstauber@solarspeed.net> 0.6.0-0BX17
- First build for 5211R
- Product set to 5211R

* Tue Oct 15 2019 Michael Stauber <mstauber@solarspeed.net> 0.6.0-0BX16
- Modified rules/defines.mk to add 'SYSTEMDUNITDIR'.

* Tue Sep 24 2019 Michael Stauber <mstauber@solarspeed.net> 0.6.0-0BX15
- Changed dependency for file-devel to file-libs, as CentOS 8 has no
  file-devel. The *.so is in file-libs, but these fuckers did not include
  the header file magic.h. So if something barfs on that we know what to
  look for. And I friggin don't get it. Fedora 30 and 31 still have
  file-devel like we've always had and here for EL8 they friggin remove
  it. 
- Added requirement for rpm-sign while we're at it.
