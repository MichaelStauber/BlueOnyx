...
# Check for hiredis (Redis/Valkey client library) - used by sessionmgr
#HAVE_HIREDIS := $(shell pkg-config --exists hiredis 2>/dev/null && echo yes || echo no)
#ifeq ($(HAVE_HIREDIS),yes)
#    HIREDIS_CFLAGS := -DUSE_REDIS $(shell pkg-config --cflags hiredis)
#    HIREDIS_LIBS := $(shell pkg-config --libs hiredis)
#else ifneq ($(wildcard /usr/include/hiredis/hiredis.h),)
#    HIREDIS_CFLAGS := -DUSE_REDIS
#    HIREDIS_LIBS := -lhiredis
#endif

# Hardcoded: Always build with Redis/Valkey:
HIREDIS_CFLAGS := -DUSE_REDIS $(shell pkg-config --cflags hiredis)
HIREDIS_LIBS := $(shell pkg-config --libs hiredis)

# For Alma 10:
#CCE_CFLAGS = -ggdb -Wcast-qual -Werror `glib-config --cflags`  -D_GNU_SOURCE
CCE_CFLAGS = -ggdb -Wcast-qual -Werror $(shell glib-config --cflags) -D_GNU_SOURCE
...
CCE_LIBS   = -L$(CCE_TOPDIR) -lcce_common $(shell glib-config --libs) -lpam -lfl
