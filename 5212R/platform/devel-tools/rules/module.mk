...
UCFIRST_VENDOR=$(shell /usr/sausalito/bin/ucfirst.pl $(VENDOR))
UCFIRST_SERVICE=$(shell /usr/sausalito/bin/ucfirst.pl $(SERVICE))
...
PRODUCT := $(shell sed -n 's/.* for a \([0-9][0-9][0-9][0-9]R\).*/\1/p' /etc/build || echo -n "5212R")

# Find out if we're using MySQL or MariaDB:
#
# All instances of XXX_DBTYPE_XXX in locales will be replaced with this:
DBTYPE := $(shell v=$$(echo $(PRODUCT) | sed 's/R//'); [ "$$v" -ge 5210 ] && echo -n "MariaDB" || echo -n "MySQL")
...
rpm: clean src_rpms mod_packing_list mod_specfile FORCE
	...
	tar --exclude '*CVS*' --exclude '*src*' --exclude '.svn' $(EXTRAEXCLUDES) -cBf - . | (cd $(TMPDIR)/$(VENDOR)-$(SERVICE)-$(VERSION); tar xBf -)
	(cd $(TMPDIR); tar czf $(RPM_TOPDIR)/SOURCES/$(VENDOR)-$(SERVICE)-$(VERSION).tar.gz $(VENDOR)-$(SERVICE)-$(VERSION))
	-rm -rf $(TMPDIR)/$(VENDOR)-$(SERVICE)-$(VERSION)
ifeq ($(SLEEP), yes)
	...
	@echo "Building RPM..."
	@$(RPMBUILD) -ta $(RPM_TOPDIR)/SOURCES/$(VENDOR)-$(SERVICE)-$(VERSION).tar.gz \
	  &> /tmp/rpm.$(PRODUCT)_$(SERVICE).log ; \
	EC=$$?; \
	echo "RPMBUILD exited with code $$EC"; \
	if [ $$EC -ne 0 ]; then \
	  echo "RPMBUILD FAILED. See /tmp/rpm.$(PRODUCT)_$(SERVICE).log"; \
	  exit $$EC; \
	fi

	@mkdir -p as_rpms
	@mkdir -p as_srpms
	-find $(RPM_TOPDIR)/RPMS  -follow -type f -newer $(TIMEFILE) -exec cp {} as_rpms \; || echo "No new RPMs copied"
	-find $(RPM_TOPDIR)/SRPMS -follow -type f -newer $(TIMEFILE) -exec cp {} as_srpms \; || echo "No new SRPMs copied"
	[ -d rpms ]  && find rpms  -type f -exec cp {} as_rpms \; || echo "No local rpms copied"
	[ -d srpms ] && find srpms -type f -exec cp {} as_srpms \; || echo "No local srpms copied"
	-@/bin/rm -f $(TIMEFILE)
	...
	if ( [ x"$(BUILDUI)" = x"yes" ] || [ x"$(BUILDUI)" = x"old" ] || [ x"$(BUILDUI)" = x"new" ] ) ; then \
		...
		if ( [ x"$(BUILDUI)" = x"yes" ] || [ x"$(BUILDUI)" = x"new" ] ) ; then \
			...
			for item in $(CHORIZO_SERVICES); do \
				echo "  -- CHORIZO VENDOR: Using Vendor: $(VENDOR) - $(UCFIRST_VENDOR)" ; \
			    mkdir -p $(CHORIZOWEB)/$(UCFIRST_VENDOR)/$(UCFIRST_SERVICE)/Config; \
			    mkdir -p $(CHORIZOWEB)/$(UCFIRST_VENDOR)/$(UCFIRST_SERVICE)/Controllers; \
			    mkdir -p $(CHORIZOWEB)/$(UCFIRST_VENDOR)/$(UCFIRST_SERVICE)/Models; \
			    mkdir -p $(CHORIZOWEB)/$(UCFIRST_VENDOR)/$(UCFIRST_SERVICE)/Views; \
			    mkdir -p $(CHORIZOWEB)/$(UCFIRST_VENDOR)/$(UCFIRST_SERVICE)/Filters; \
				FLIST=`find $$dir/$$item/Config -follow -type f 2>/dev/null \
					| grep -v CVS | grep -v '/\.' | grep -v '/\.svn' `; \
				if [ -n "$$FLIST" ]; then \
				    echo "  -- installing $$item" ; \
				    echo "  -- FILELIST: $$FLIST" ; \
				    $(INSTALL_OTH) $$FLIST $(CHORIZOWEB)/$(UCFIRST_VENDOR)/$(UCFIRST_SERVICE)/Config; \
				fi; \
				...
				if [ -n "$$FLIST" ]; then \
				    echo "  -- installing $$item" ; \
				    echo "  -- FILELIST: $$FLIST" ; \
				    $(INSTALL_OTH) $$FLIST $(CHORIZOWEB)/$(UCFIRST_VENDOR)/$(UCFIRST_SERVICE)/Controllers; \
				fi; \
				...
				if [ -n "$$FLIST" ]; then \
				    echo "  -- installing $$item" ; \
				    echo "  -- FILELIST: $$FLIST" ; \
				    $(INSTALL_OTH) $$FLIST $(CHORIZOWEB)/$(UCFIRST_VENDOR)/$(UCFIRST_SERVICE)/Models; \
				fi; \
				...
				if [ -n "$$FLIST" ]; then \
				    echo "  -- installing $$item" ; \
				    echo "  -- FILELIST: $$FLIST" ; \
				    $(INSTALL_OTH) $$FLIST $(CHORIZOWEB)/$(UCFIRST_VENDOR)/$(UCFIRST_SERVICE)/Views; \
				fi; \
				FLIST=`find $$dir/$$item/Filters -follow -type f 2>/dev/null \
					| grep -v CVS | grep -v '/\.' | grep -v '/\.svn' `; \
				if [ -n "$$FLIST" ]; then \
				    echo "  -- installing $$item" ; \
				    echo "  -- FILELIST: $$FLIST" ; \
				    $(INSTALL_OTH) $$FLIST $(CHORIZOWEB)/$(UCFIRST_VENDOR)/$(UCFIRST_SERVICE)/Filters; \
				fi; \
			done; \
