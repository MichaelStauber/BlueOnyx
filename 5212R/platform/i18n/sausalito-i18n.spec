%define admserv_php_prefix /home/solarspeed/admserv-php
%define admserv_extension_dir %{admserv_php_prefix}/lib/php/20230831
%define admserv_php_ini_dir %{admserv_php_prefix}/etc/php.d
...
Requires: admserv-php
BuildRequires: admserv-php
...
%{admserv_extension_dir}/i18n.so
%{admserv_php_ini_dir}/i18n.ini
...
if [ -x /bin/systemctl ]; then
	/bin/systemctl try-restart admserv-php-fpm.service >/dev/null 2>&1 || :
fi

%changelog

* Sat Apr 25 2026 Michael Stauber <mstauber@solarspeed.net> 0.82.3-2
- Re-enabled the native i18n PHP extension for AdmServ PHP 8.3.
- The RPM now builds and installs i18n.so into
  /home/solarspeed/admserv-php/lib/php/20230831/.
- Added /home/solarspeed/admserv-php/etc/php.d/i18n.ini to enable the
  extension automatically for AdmServ PHP.
- Packaging and build rules now use AdmServ PHP's phpize/php-config
  instead of the system PHP extension directory.

* Sat Jun 07 2025 Michael Stauber <mstauber@solarspeed.net> 0.82.1-1
- Removed embedded RPATH from libi18n.so.0.81 to comply with RPM 
  packaging guidelines
- Updated Makefile and build rules to avoid -Wl,-rpath and use 
  -Wl,-rpath-link instead
- Added missing #include <stdlib.h> to i18n_cmndline.c, 
  i18n_translate.c, and i18n_locales.c to fix implicit declaration 
  errors on systems using modern GCC

* Mon Feb 19 2024 Michael Stauber <mstauber@solarspeed.net> 0.81.3-0BX03
- Version number bump for Elmer release

* Wed Nov 17 2022 Michael Stauber <mstauber@solarspeed.net> 0.81.3-0BX02
- Release version for BlueOnyx 5211R.
- Removed Cracklib support. Although I got it working for PHP-8.0 with a
  hell of a lot of effort, there is no way I'll do this again for PHP-8.1
  Anyway, the GUI no longer uses Cracklib, so we're good.

* Wed Nov 10 2021 Michael Stauber <mstauber@solarspeed.net> 0.81.3-0BX01
- First build for 5211R on RHEL9 Beta.
- Fixed cracklib with patches from Gentoo to make it PHP-8.0 compatible
- The i18n PHP module remains disabled, as we don't need it anymore.
...
- Disabled building of i18n PHP module
